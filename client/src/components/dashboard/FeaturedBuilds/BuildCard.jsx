...
export default function BuildCard({build}) {
    const { name, price, image, components = [] } = build;
    const navigate = useNavigate();

    const getComponent = (category) => components.find(
        (component) => component.inventoryId?.category === category
    )?.inventoryId;

    const cpu = getComponent("CPU");
    const gpu = getComponent("GPU");
    const ram = getComponent("RAM");
